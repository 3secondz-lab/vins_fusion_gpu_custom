...
void registerPub(ros::NodeHandle &n, int queue_size)
{
    pub_latest_odometry = n.advertise<nav_msgs::Odometry>("imu_propagate", queue_size);
    pub_path = n.advertise<nav_msgs::Path>("path", queue_size);
    pub_odometry = n.advertise<nav_msgs::Odometry>("odometry", queue_size);
    pub_point_cloud = n.advertise<sensor_msgs::PointCloud>("point_cloud", queue_size);
    pub_margin_cloud = n.advertise<sensor_msgs::PointCloud>("margin_cloud", queue_size);
    pub_key_poses = n.advertise<visualization_msgs::Marker>("key_poses", queue_size);
    pub_camera_pose = n.advertise<nav_msgs::Odometry>("camera_pose", queue_size);
    pub_camera_pose_right = n.advertise<nav_msgs::Odometry>("camera_pose_right", queue_size);
    pub_rectify_pose_left = n.advertise<geometry_msgs::PoseStamped>("rectify_pose_left", queue_size);
    pub_rectify_pose_right = n.advertise<geometry_msgs::PoseStamped>("rectify_pose_right", queue_size);
    pub_camera_pose_visual = n.advertise<visualization_msgs::MarkerArray>("camera_pose_visual", queue_size);
    pub_keyframe_pose = n.advertise<nav_msgs::Odometry>("keyframe_pose", queue_size);
    pub_keyframe_point = n.advertise<sensor_msgs::PointCloud>("keyframe_point", queue_size);
    pub_extrinsic = n.advertise<nav_msgs::Odometry>("extrinsic", queue_size);
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        std::cout << "Accel Bias : " << estimator.latest_Ba.transpose() << std::endl;
        std::cout << "Gyro Bias : " << estimator.latest_Bg.transpose() << std::endl;
    }
}
